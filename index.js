...
app.post("/swap", async (req, res) => {
  try {
    ...
    const userOldToken = await getOrCreateAssociatedTokenAccount(
      connection,
      OWNER,
      OLD_DRD_MINT,
      userPubkey
    ...
    const userNewToken = await getOrCreateAssociatedTokenAccount(
      connection,
      OWNER,
      NEW_DRD_MINT,
      userPubkey
    ...
    const ownerNewToken = await getOrCreateAssociatedTokenAccount(
      connection,
      OWNER,
      NEW_DRD_MINT,
      OWNER.publicKey
    ...
    if (oldAmount < 20 * 1e6)
      return res.status(400).json({ error: "Not enough OLD DRD to swap" });

    const newAmount = Math.floor(oldAmount / (20 * 1e6)) * 1e6;

    if (totalDistributed + newAmount > MAX_NEW_DRD)
      return res.status(400).json({ error: "Airdrop cap reached" });

    const tx = new Transaction().add(
      transfer(
        ownerNewToken.address,
        userNewToken.address,
        OWNER.publicKey,
        newAmount,
        [],
        TOKEN_PROGRAM_ID
...
app.listen(port, () => {
  console.log(`🚀 DRD Swap backend running on port ${port}`);
});
